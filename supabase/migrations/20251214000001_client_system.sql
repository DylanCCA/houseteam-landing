-- Client Notification & Lead Management System (Standalone)
-- For The House Team - Automated listing alerts, showing scheduler, admin dashboard
-- Version: 1.0.1 - No external dependencies
...
CREATE TABLE IF NOT EXISTS client_profiles (
  ...
  assigned_agent_id UUID, -- No FK, just store the ID
...
CREATE INDEX IF NOT EXISTS idx_showings_date ON scheduled_showings(scheduled_date);
...
-- Allow anon to insert new clients (signup form)
CREATE POLICY "Allow anon insert clients" ON client_profiles
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Allow anon insert criteria" ON client_criteria
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Allow anon read showings" ON scheduled_showings
  FOR SELECT USING (true);

CREATE POLICY "Allow anon insert showings" ON scheduled_showings
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Allow anon update showings" ON scheduled_showings
